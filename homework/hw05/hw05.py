class VendingMachine:
    ...
    'Here is your soda.'
    """
    def __init__(self, product, price,stock = 0,money = 0):
...
def is_bst(t):
    ...
    if t.is_leaf():
        ...
        if len(bs) == 1:
            ...
            con1 = is_bst(t.branches[0])
            con2 = bst_max(left) <= t.label or bst_min(right) > t.label
            return  con1 and con2
        elif len(bs) == 2:
            left,right= t.branches[0],t.branches[1]
            con1 = is_bst(left)
            con2 = is_bst(right)
            con3 = bst_max(left) <= t.label and bst_min(right) > t.label
    ...
    if t.label == value:
        ...
        for b in t.branches:
            for path in path_yielder(b,value):
                yield '{'+str(t.label)+'}' + str(path) + '{yield from}'
