...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operands = rest.map(lambda x:scheme_eval(x,env)) # try to understand this line.
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        print("DEBUG:", fn_name)
        formulas = target.rest
        body = expressions.rest
        new_exp = Pair(formulas,body)
        env.define(fn_name, do_lambda_form(new_exp,env))
        return fn_name
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    while expressions:
        ...
        value = is_true_primitive(evaled)
        if expressions.rest is nil and value:
                return evaled
        elif value:
            expressions = expressions.rest
        else:
