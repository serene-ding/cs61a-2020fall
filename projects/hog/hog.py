...
def swine_align(player_score, opponent_score):
    ...
    swine = True
    while swine:
        if player_score % smaller == 0 and opponent_score % smaller == 0:
            swine = False    
        else:
            smaller -= 1
    if smaller < 10:
...
def pig_pass(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4b
    "*** YOUR CODE HERE ***"
    if player_score >= opponent_score or opponent_score - player_score >= 3:
        return False
    else:
        return True
    # END PROBLEM 4b
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    while n <= 10 :
        ...
        max_score_n = average_roll_dice(n,dice)
...
def run_experiments():
    ...
    if False:  # Change to False when done finding max_scoring_num_rolls
        max_ = max_scoring_num_rolls(make_test_dice(5))
        print('Max scoring num rolls for dice:', max_)
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if extra_turn(score + gain, opponent_score):
        num_rolls = 0
    elif gain >= cutoff:
        num_rolls = 0
    return num_rolls  # Replace this statement
