    ...
    def nearest_bee(self, beehive):
        ...
        while current is not beehive :
            ...
            if min_range <= count <= max_range:
                con = rANTdom_else_none(current.bees)
                if con:
                    return con
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        """
        if self.count == 0:
            ScubaThrower.throw_at(self,ScubaThrower.nearest_bee(self,gamestate.beehive))
            b_place = self.place.exit
            while b_place:
                if b_place.ant and not b_place.ant.doubled :
                    b_place.ant.damage = 2*b_place.ant.damage
                    b_place.ant.doubled = True
                b_place = b_place.exit
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem Optional 2
        con = isinstance(other,ContainerAnt)
        if not self.contained_ant and not con :
            return True
        return False
        # END Problem Optional 2
...
def apply_status(status, bee, length):
    ...
    if length >= 0:
        bee.action = status(old_action,bee)
        length -= 1 
    else:
